...
class GetMasterBeautyServiceUseCase:

    def __init__(self, repo: BeautyServiceRepository):

        self.repo = repo

    async def execute(self, beauty_service_id: int,
                      master_id: int) -> BeautyServiceDto:

        beauty_service: BeautyService = await self.repo.get_master_beauty_service(beauty_service_id=beauty_service_id,
                                                            master_id=master_id)
